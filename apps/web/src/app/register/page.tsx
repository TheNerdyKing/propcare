...
export default function RegisterPage() {
    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-[#334155] py-8 px-4 font-sans relative overflow-hidden">
            <div className="absolute top-0 left-0 w-full h-full bg-[radial-gradient(circle_at_30%_30%,#475569_0%,transparent_60%)] pointer-events-none" />
            <div className="absolute top-1/4 left-1/4 w-[400px] h-[400px] bg-blue-500/10 rounded-full blur-[100px] pointer-events-none" />

            <div className="max-w-[20rem] w-full space-y-6 p-6 rounded-[1.5rem] border border-white/5 bg-blue-950/60 backdrop-blur-3xl shadow-[0_0_30px_rgba(0,0,0,0.2)] ring-1 ring-white/10 relative overflow-hidden">
                <div className="absolute top-0 right-0 w-32 h-32 bg-blue-600/10 rounded-full -translate-y-1/2 translate-x-1/2 blur-2xl pointer-events-none" />

                <div className="relative z-10 text-center mb-4 overflow-visible">
                    <Logo light showStatus className="mx-auto" />
                </div>
                ...
                {setupSecret ? (
                    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4">
                        <div className="text-center">
                            ...
                            <h2 className="text-2xl font-black text-white uppercase tracking-tighter">Erfolgreich!</h2>
                            <p className="text-slate-500 text-xs mt-1 font-medium italic">Account erstellt. Ihr Zugangsschlüssel:</p>
                        </div>

                        <div className="bg-emerald-500/5 border border-emerald-500/20 rounded-xl p-5 text-center ring-1 ring-emerald-500/10">
                            <p className="text-[8px] font-black uppercase tracking-[0.2em] text-emerald-500/60 mb-1">Setup-Code</p>
                            <p className="text-xl font-mono font-black text-white tracking-widest bg-emerald-500/10 py-2.5 rounded-lg border border-emerald-500/30">{setupSecret}</p>
                        </div>

                        <div className="space-y-3">
                            <Link
                                href="/login"
                                className="w-full h-11 bg-blue-600 hover:bg-blue-500 text-white rounded-lg font-black uppercase tracking-[0.2em] text-[9px] shadow-[0_0_15px_rgba(37,99,235,0.2)] flex items-center justify-center transition-all"
                            >
                                Jetzt Anmelden <ArrowRight className="w-3.5 h-3.5 ml-2" />
                            </Link>
                            <p className="text-[8px] text-slate-500 text-center italic leading-relaxed uppercase tracking-widest px-4">
                                Code für den ersten Login sicher aufbewahren.
                            </p>
                        </div>
                    </div>
                ) : (
                    <form className="mt-10 space-y-5" onSubmit={handleSubmit(onSubmit)}>
                        ...
                        <div className="space-y-3 font-sans">
                            <div className="relative group">
                                ...
                                <input
                                    ...
                                    className="w-full pl-9 pr-3 py-2.5 bg-white/5 border border-white/10 rounded-lg text-white placeholder-slate-600 focus:outline-none focus:ring-2 focus:ring-blue-500/10 focus:border-blue-500/40 transition-all font-bold text-[10px] shadow-inner"
                                />
                                {errors.tenantName && (
                                    <p className="text-red-400 text-[8px] font-black uppercase mt-1 ml-1 tracking-widest">{errors.tenantName.message}</p>
                                )}
                            </div>
                        ...
                        <div className="pt-4">
                            <button
                                type="submit"
                                disabled={loading}
                                className="w-full h-11 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-black uppercase tracking-[0.2em] text-[9px] shadow-[0_0_15px_rgba(37,99,235,0.2)] flex items-center justify-center transition-all hover:scale-[1.02] active:scale-[0.98] disabled:opacity-50 group font-sans"
                            >
